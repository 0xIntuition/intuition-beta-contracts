...
contract EthMultiVault is IEthMultiVault, Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function getDepositFees(uint256 assets, uint256 id) public view returns (uint256 totalFees) {
        uint256 protocolFees = protocolFeeAmount(assets, id);

        totalFees = entryFeeAmount(assets, id) + atomEquityFeeAmount(assets - protocolFees, id) + protocolFees;
    }
    ...
    function _createAtom(bytes memory atomUri, uint256 value)
        internal
        returns (uint256 id, uint256 protocolDepositFee)
    {
        ...
        returns (uint256 id)
    {
        ...
        (id, protocolDepositFee) = _createTriple(subjectId, predicateId, objectId, msg.value);
    ...
    function redeemAtom(uint256 shares, address receiver, uint256 id) external nonReentrant returns (uint256 assets) {
        if (id == 0 || id > count) {
            revert Errors.MultiVault_VaultDoesNotExist();
        }
    ...
    function _depositIntoVault(
        ...
        uint256 totalAssetsDelta = assets - entryFeeAmount(assets, id) - atomEquityFeeAmount(assets, id);
    ...
    function _deposit(address receiver, uint256 id, uint256 value)
        internal
        returns (uint256 shares, uint256 protocolFees)
    {
