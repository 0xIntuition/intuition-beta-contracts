...
/**
 * @title  EthMultiVaultExperimental
 * @author 0xIntuition
 * @notice EthMultiVaultExperimental contract with methods that can be overridden for testing purposes
 */
contract EthMultiVaultExperimental is IEthMultiVault, Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function setAdmin(address admin) external onlyAdmin {
        ...
        // Generate the operation hash
        bytes memory data = abi.encodeWithSelector(IEthMultiVault.setAdmin.selector, admin);
    ...
    function setExitFee(uint256 id, uint256 exitFee) external onlyAdmin {
        ...
        // Generate the operation hash
        bytes memory data = abi.encodeWithSelector(IEthMultiVault.setExitFee.selector, id, exitFee);
    ...
    function currentSharePrice(uint256 id) external view virtual returns (uint256) {
    ...
    function convertToShares(uint256 assets, uint256 id) public view virtual returns (uint256) {
        uint256 supply = vaults[id].totalShares;
